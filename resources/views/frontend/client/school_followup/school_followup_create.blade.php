...
@endauth
    ...
    <div class="card shadow-sm border-secondary">
        ...
        <div class="card-header bg-dark text-white py-2 px-3 position-relative text-center">
            ...
            <!-- ปุ่ม toggle followup -->
            <button id="toggleFollowupBtn"
                    class="btn btn-sm btn-light position-absolute end-0 top-50 translate-middle-y me-3 d-flex align-items-center"
                    type="button"
                    data-bs-toggle="collapse"
                    data-bs-target="#followupForm"
                    aria-expanded="{{ isset($followup) ? 'true' : 'false' }}"
                    aria-controls="followupForm">
                <i class="bi {{ isset($followup) ? 'bi-chevron-up' : 'bi-chevron-down' }}"></i>
                <span class="ms-1">
                    {{ isset($followup) ? 'ซ่อน/ฟอร์ม' : 'เพิ่มข้อมูล' }}
                </span>
            </button>
        </div>
        ...
        <div id="followupForm" class="collapse {{ isset($followup) ? 'show' : '' }}">
            <div class="card-body p-3">
                <form action="{{ isset($followup) ? route('school_followup.update', $followup->id) : route('school_followup_store') }}"
                      method="POST">
                    @csrf
                    @if(isset($followup))
                        @method('PUT')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (collapseEl && toggleBtn) {
            collapseEl.addEventListener('shown.bs.collapse', function () {
                ...
                toggleBtn.querySelector('span').textContent = 'ซ่อน/ฟอร์ม';
            });

            collapseEl.addEventListener('hidden.bs.collapse', function () {
                toggleBtn.querySelector('i').className = 'bi bi-chevron-down';
                toggleBtn.querySelector('span').textContent = 'เพิ่มข้อมูล';
            });
        }
    });
</script>
