        ...
        <div class="card-header bg-white border-0 py-3 px-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    ...
                    <div class="text-muted small">กำหนดข้อมูลผู้ใช้งาน หน่วยงาน และบ้านที่ดูแล</div>
                </div>
            ...
            <form action="{{ route('users.store') }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-lg-6">
                        ...
                        <input type="text" name="name" class="form-control form-control-modern" value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="col-lg-6">
                        ...
                        <input type="email" name="email" class="form-control form-control-modern" value="{{ old('email') }}">
                        @error('email')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-lg-6">
                        ...
                        <select name="role" class="form-select form-control-modern">
                            ...
                            <option value="manager" {{ old('role') == 'manager' ? 'selected' : '' }}>ผู้ใช้ / เจ้าหน้าที่</option>
                            <option value="executive" {{ old('role') == 'executive' ? 'selected' : '' }}>ผู้บริหาร</option>
                            <option value="social_worker" {{ old('role') == 'social_worker' ? 'selected' : '' }}>นักสังคมสงเคราะห์</option>
                            <option value="teacher_caregiver" {{ old('role') == 'teacher_caregiver' ? 'selected' : '' }}>ครู/ผู้ดูแล</option>
                            <option value="nurse" {{ old('role') == 'nurse' ? 'selected' : '' }}>พยาบาล</option>
                            <option value="general_user" {{ old('role') == 'general_user' ? 'selected' : '' }}>ผู้ใช้ทั่วไป</option>
                        </select>
                        @error('role')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                                    ...
                                    <div class="col-md-6 col-xl-4">
                                        <label class="house-option" for="house_{{ $house->id }}">
                                            <input
                                                ...
                                                value="{{ $house->id }}"
                                                id="house_{{ $house->id }}"
                                                {{ in_array($house->id, old('house_ids', [])) ? 'checked' : '' }}
                                            >
